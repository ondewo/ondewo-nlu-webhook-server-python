...
class TestMakeHttpRequest:
    ...
    async def test_get_success(self) -> None:
        respx.get("https://example.com/api").mock(return_value=httpx.Response(200, json={"key": "value"}))
    ...
    async def test_post_with_json_payload(self) -> None:
        respx.post("https://example.com/api").mock(return_value=httpx.Response(201, json={"created": True}))
    ...
    async def test_post_without_payload(self) -> None:
        ...
        result = await make_http_request("post", "https://example.com/api", headers={}, params={"q": "1"})
    ...
    async def test_delete_success(self) -> None:
        respx.delete("https://example.com/api/1").mock(return_value=httpx.Response(200, json={"deleted": True}))
        result = await make_http_request("delete", "https://example.com/api/1", headers={})
        assert result == {"deleted": True}

    @respx.mock
    @pytest.mark.asyncio
    async def test_put_success(self) -> None:
        respx.put("https://example.com/api/1").mock(return_value=httpx.Response(200, json={"updated": True}))
        result = await make_http_request(
            "put",
    ...
    async def test_server_error_status(self) -> None:
        respx.get("https://example.com/api").mock(return_value=httpx.Response(500, text="Internal Server Error"))
        with pytest.raises(HTTPException) as exc_info:
